...
# Este script automatiza la generación de certificados SSL.
# Se espera que se ejecute desde el directorio /home/jestebanl/certificadosSSL/
# y que el archivo template.cnf esté en el mismo directorio.
...
# Sanitizar el nombre de dominio para usarlo en nombres de archivo (reemplazar puntos con guiones bajos)
FILE_NAME=$(echo "$DOMAIN_NAME" | sed 's/\./_/g')
YEAR=$(date +%Y)
DOMAIN_DIR="/home/jestebanl/certificadosSSL/$FILE_NAME"
...
# Copiar el contenido del .csr al portapapeles (requiere xclip para sistemas basados en X11)
if command -v xclip &> /dev/null; then
    echo "$CSR_CONTENT" | xclip -selection clipboard
    echo ""
    echo "¡El contenido del CSR ha sido copiado a su portapapeles! Puede pegarlo directamente en la interfaz de HARICA."
    echo ""
else
    echo "Advertencia: 'xclip' no está instalado. No se pudo copiar el contenido al portapapeles."
