#!/bin/bash
# VERSIÓN 12.0 - SCRIPT DEDICADO Y SEGURO PARA AÑADIR REGISTROS A+PTR
...
ssh -T "${DNS_USER}@${DNS_MASTER}" << EOF || ssh_exit_status=$?
    ...
    if [[ "\$ACTION" == "ADD" ]]; then
        ...
        sed -i "s/^\s*${LAST_OCTET}\s\+IN\s\+PTR\s\+${OLD_FQDN_OF_PTR}\..*$/${LAST_OCTET}\t\tIN\tPTR\t${FQDN}./" "\$REV_FILE"
    fi
...
if [ $? -eq 0 ]; then
    ...
    echo "Error remoto (código de salida: $?)."
fi
