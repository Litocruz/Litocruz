...
# VERSIÓN:      6.0 (Versión final completa con todas las opciones)
# DESCRIPCIÓN:  Añade, modifica o renombra registros A y PTR en un servidor BIND.
# ==============================================================================
...
echo "---------------------------------"
echo "1) Añadir un nuevo registro (o reasignar una IP)"
echo "2) Modificar la IP de un registro existente"
echo "3) Renombrar un registro existente"
read -p "Elige una opción [1-3]: " choice

# Inicializamos variables
ACTION=""; FQDN=""; IP=""; NEW_FQDN=""; OLD_IP=""; OLD_FQDN_OF_PTR=""
...
if [ "$ACTION" == "ADD" ]; then
    if [ -n "$OLD_IP_OF_FQDN" ]; then echo "Error: El registro A para '$FQDN' ya existe y apunta a $OLD_IP_OF_FQDN."; exit 1; fi
    ...
    if [ -n "$PTR_RECORD" ]; then
        ...
        read -p "¿Quieres forzar la reasignación de esta IP al nuevo host '$FQDN'? (Esto modificará el PTR existente) (s/n): " CONFIRM_REASSIGN
    ...
    if [ -z "$OLD_IP_OF_FQDN" ]; then echo "Error: El registro para '$FQDN' no existe. No se puede modificar."; exit 1; fi
    ...
    if [ -z "$OLD_IP_OF_FQDN" ]; then echo "Error: El registro para '$FQDN' no existe. No se puede renombrar."; exit 1; fi
    IP=$OLD_IP_OF_FQDN
...
ssh -T "${DNS_USER}@${DNS_MASTER}" << EOF || ssh_exit_status=$?
    ...
    case "\$ACTION" in
        ADD|REASSIGN_PTR)
            ...
            REVERSE_ZONE_FILE="\${ZONE_PATH}/db.\$(echo "\$IP" | cut -d'.' -f1-3)"
            if [ ! -f "\$FORWARD_ZONE_FILE" ]; then echo "Error: Fichero de zona directa no encontrado."; exit 1; fi
            if [ ! -f "\$REVERSE_ZONE_FILE" ]; then echo "Error: El fichero de zona inversa '\$REVERSE_ZONE_FILE' no existe."; exit 1; fi
            ...
            if [ "\$ACTION" == "ADD" ]; then
                ...
                echo "Modificando registro PTR existente..."; sed -i "/\s\+PTR\s\+${OLD_FQDN_OF_PTR}\./ s/IN.*/IN\tPTR\t${FQDN}./" "\$REVERSE_ZONE_FILE"
            fi
            ;;
        MODIFY)
            FORWARD_ZONE_FILE="\${ZONE_PATH}/\$(find_forward_zone_file "\$FQDN")"
            OLD_REVERSE_ZONE_FILE="\${ZONE_PATH}/db.\$(echo "\$OLD_IP" | cut -d'.' -f1-3)"
            NEW_REVERSE_ZONE_FILE="\${ZONE_PATH}/db.\$(echo "\$IP" | cut -d'.' -f1-3)"
            files_to_update["\$FORWARD_ZONE_FILE"]=1; files_to_update["\$OLD_REVERSE_ZONE_FILE"]=1; files_to_update["\$NEW_REVERSE_ZONE_FILE"]=1
            echo "Creando backups..."; for file in "\${!files_to_update[@]}"; do if [ -f "\$file" ] && [ ! -f "\${file}-\${BACKUP_SUFFIX}" ]; then cp -p "\$file" "\${file}-\${BACKUP_SUFFIX}"; fi; done
            echo "Modificando registro A..."; HOSTNAME=\$(echo "\$FQDN" | cut -d'.' -f1)
            sed -i "s/^\(${HOSTNAME}\s\+.*\s\+A\s\+\)\${OLD_IP}/\1\${IP}/" "\$FORWARD_ZONE_FILE"
            echo "Borrando registro PTR antiguo..."; sed -i "/\s\+PTR\s\+${FQDN}./d" "\$OLD_REVERSE_ZONE_FILE"
            echo "Añadiendo nuevo registro PTR..."; LAST_OCTET=\$(echo "\$IP" | awk -F. '{print \$4}')
            inserted=false; for i in \$(seq \$((LAST_OCTET - 1)) -1 1); do if grep -q -E "^\s*\${i}\s+" "\$NEW_REVERSE_ZONE_FILE"; then sed -i "/^\s*\${i}\s\+/a \${LAST_OCTET}\t\tIN\tPTR\t\${FQDN}." "\$NEW_REVERSE_ZONE_FILE"; inserted=true; break; fi; done; if [ "\$inserted" = false ]; then echo -e "\${LAST_OCTET}\t\tIN\tPTR\t\${FQDN}." >> "\$NEW_REVERSE_ZONE_FILE"; fi
            ;;
        RENAME)
            FORWARD_ZONE_FILE="\${ZONE_PATH}/\$(find_forward_zone_file "\$FQDN")"
            REVERSE_ZONE_FILE="\${ZONE_PATH}/db.\$(echo "\$IP" | cut -d'.' -f1-3)"
            files_to_update["\$FORWARD_ZONE_FILE"]=1; files_to_update["\$REVERSE_ZONE_FILE"]=1
            echo "Creando backups..."; for file in "\${!files_to_update[@]}"; do cp -p "\$file" "\${file}-\${BACKUP_SUFFIX}"; done
            echo "Renombrando registro A..."; OLD_HOSTNAME=\$(echo "\$FQDN" | cut -d'.' -f1); NEW_HOSTNAME=\$(echo "\$NEW_FQDN" | cut -d'.' -f1)
            sed -i "s/^${OLD_HOSTNAME}\s\+/${NEW_HOSTNAME}\t/" "\$FORWARD_ZONE_FILE"
            echo "Renombrando registro PTR...";
            sed -i "s/\s\+PTR\s\+${FQDN}./\tIN\tPTR\t${NEW_FQDN}./" "\$REVERSE_ZONE_FILE"
            ;;
    esac

    for file in "\${!files_to_update[@]}"; do if [ ! -f "\$file" ]; then continue; fi; echo "Actualizando SOA para \$file..."; NEW_SOA=\$(calculate_new_soa "\$file"); sed -i.bak -E "/IN\s+SOA\s\+/,/\)/s/^\s*[0-9]{10,}/\t\t\${NEW_SOA}/" "\$file"; rm -f "\${file}.bak"; done
    echo "Validando sintaxis..."; for file in "\${!files_to_update[@]}"; do if [ ! -f "\$file" ]; then continue; fi; zone_domain=\$(echo "\$file" | sed -E 's#.*/db\.(.*)#\1#'); if [[ \$zone_domain =~ ^[0-9] ]]; then zone_name=\$(echo "\$zone_domain" | awk -F. '{print \$3"."\$2"."\$1".in-addr.arpa"}'); else zone_name=\$zone_domain; fi; if ! named-checkzone "\$zone_name" "\$file" > /dev/null 2>&1; then echo "¡ERROR DE SINTAXIS EN \$file! Restaurando..."; for f_restore in "\${!files_to_update[@]}"; do if [ -f "\${f_restore}-\${BACKUP_SUFFIX}" ]; then mv "\${f_restore}-\${BACKUP_SUFFIX}" "\$f_restore"; fi; done; exit 1; fi; done
    echo "Validación correcta."; echo "Recargando master..."; if ! systemctl reload named; then echo "¡FALLO AL RECARGAR MASTER!"; exit 1; fi; echo "Recargando esclavo..."; if ! ssh "\$DNS_SLAVE" "systemctl reload named"; then echo "¡AVISO! Falló la recarga en esclavo."; fi
    echo "Limpiando backups..."; for file in "\${!files_to_update[@]}"; do if [ -f "\$file" ]; then mkdir -p "\${ZONE_PATH}/old"; mv "\${file}-\${BACKUP_SUFFIX}" "\${ZONE_PATH}/old/"; fi; done
...
if [ $? -eq 0 ]; then
    ...
    if [ "$ACTION" == "RENAME" ]; then
        LOG_MSG="$(date '+%Y-%m-%d %H:%M:%S') - $LOG_ACTION - FROM: $FQDN, TO: $NEW_FQDN, IP: $IP - Usuario: $USER"
    else
        LOG_MSG="$(date '+%Y-%m-%d %H:%M:%S') - $LOG_ACTION - FQDN: $FQDN, IP: $IP, Old IP: $OLD_IP - Usuario: $USER"
    fi
    echo "$LOG_MSG" >> "$LOG_FILE"
    
    echo "Verificando los cambios:"
    sleep 3
    if [ "$ACTION" == "RENAME" ]; then FQDN=$NEW_FQDN; fi
    dig "$FQDN" @"$DNS_MASTER" | grep --color=never -A1 "ANSWER SECTION"
else
