#!/bin/bash
# VERSIÓN 12.1 - SCRIPT DEDICADO Y SEGURO PARA AÑADIR REGISTROS A+PTR
# Con método de paso de variables robusto y comando de edición verificado.
set -e
...
ssh -T "${DNS_USER}@${DNS_MASTER}" << EOF
    ...
    if [[ ! -f "\$FWD_FILE" ]] || [[ ! -f "\$REV_FILE" ]]; then echo "Error: Fichero de zona no encontrado."; exit 1; fi

    FILES_TO_UPDATE=("\$FWD_FILE" "\$REV_FILE")
    BACKUP_SUFFIX=\$(date +"%Y%m%d-%H%M%S")
    
    echo "Creando backups..."
    for f in "\${FILES_TO_UPDATE[@]}"; do cp -p "\$f" "\${f}-\${BACKUP_SUFFIX}"; done
    
    HOSTNAME=\$(echo "\$FQDN" | cut -d'.' -f1); LAST_OCTET=\$(echo "\$IP" | awk -F. '{print \$4}')
    ...
    if [ "\$inserted" = false ]; then echo -e "\${HOSTNAME}\t\tIN\tA\t\${IP}" >> "\$FWD_FILE"; fi
    
    # Añadir o modificar registro PTR
    if [[ "\$ACTION" == "ADD" ]]; then
        inserted=false
        ...
        if [ "\$inserted" = false ]; then echo -e "\${LAST_OCTET}\t\tIN\tPTR\t\${FQDN}." >> "\$REV_FILE"; fi
    else # REASSIGN_PTR
        echo "Modificando registro PTR existente..."
        # MEJORA: Escapar puntos para una expresión regular segura
        OLD_FQDN_OF_PTR_ESC=\$(echo "\$OLD_FQDN_OF_PTR" | sed 's/\\./\\\\./g')
        # CORRECCIÓN: Usar variables escapadas para que se evalúen en el servidor remoto
        sed -i "s/^\s*\${LAST_OCTET}\s\+IN\s\+PTR\s\+\${OLD_FQDN_OF_PTR_ESC}\..*$/\${LAST_OCTET}\t\tIN\tPTR\t\${FQDN}./" "\$REV_FILE"
    fi

    # Actualizar SOA
    for f in "\${FILES_TO_UPDATE[@]}"; do NEW_SOA=\$(calc_soa "\$f"); sed -i.bak -E "/IN\s+SOA\s\+/,/\)/s/^\s*[0-9]{10,}/\t\t\${NEW_SOA}/" "\$f"; rm -f "\${f}.bak"; done

    # Validar
    echo "Validando..."
    for f in "\${FILES_TO_UPDATE[@]}"; do
        domain=\$(echo "\$f" | sed -E 's#.*/db\\.(.*)#\\1#')
        if [[ \$domain =~ ^[0-9] ]]; then name=\$(echo "\$domain" | awk -F. '{print \$3"."\$2"."\$1".in-addr.arpa"}'); else name=\$domain; fi
        if ! named-checkzone "\$name" "\$f" > /dev/null 2>&1; then
            echo "ERROR DE SINTAXIS EN \$f. Restaurando backups...";
            for fr in "\${FILES_TO_UPDATE[@]}"; do mv "\${fr}-\${BACKUP_SUFFIX}" "\$fr"; done
            exit 1
        fi
    done

    echo "Recargando servicio (reload)..."; systemctl reload named
    if ! ssh -o ConnectTimeout=5 "\$DNS_SLAVE" "systemctl reload named" &>/dev/null; then echo "AVISO: Fallo la recarga en el esclavo."; fi
    echo "Limpiando backups..."; for f in "\${FILES_TO_UPDATE[@]}"; do mkdir -p "\${ZONE_PATH}/old"; mv "\${f}-\${BACKUP_SUFFIX}" "\${ZONE_PATH}/old/"; done
EOF
# --- FIN DEL BLOQUE SSH ---

ssh_exit_status=$? # CAPTURA CORRECTA DEL CÓDIGO DE SALIDA
...
if [ $ssh_exit_status -eq 0 ]; then
    ...
    echo "Error remoto (código de salida: $ssh_exit_status)."
fi
