...
ssh -T "${DNS_USER}@${DNS_MASTER}" << EOF
    ...
    # --- Creación de Backups (REFACTORIZADO) ---
    ...
    # Se busca la IP anterior para insertar el nuevo registro de forma ordenada.
    ...
    if [[ "\$ACTION" == "ADD" ]]; then
        # Misma lógica ordenada para el PTR
        inserted=false
    ...
    # 3. Actualizar Serial (SOA) en ambos ficheros (REFACTORIZADO)
    echo "Actualizando números de serie (SOA)..."
    for f in "\${FILES_TO_UPDATE[@]}"; do
        NEW_SOA=\$(calc_soa "\$f")
        # -i.bak crea un backup que luego eliminamos para mayor seguridad
    ...
    for f in "\${FILES_TO_UPDATE[@]}"; do
        ...
        if ! named-checkzone "\$name" "\$f" > /dev/null 2>&1; then
            ...
            # Bucle de restauración (REFACTORIZADO)
            for fr in "\${FILES_TO_UPDATE[@]}"; do
                mv "\${fr}-\${BACKUP_SUFFIX}" "\$fr"
            done
            exit 1
        fi
    done
    ...
    if ! ssh -n -o ConnectTimeout=5 "\$DNS_SLAVE" "systemctl reload named" &>/dev/null; then 
        echo "AVISO: Falló la recarga en el servidor esclavo \$DNS_SLAVE."
    fi
    
    # Limpieza de backups (REFACTORIZADO)
    echo "Limpiando backups..."
    mkdir -p "\${ZONE_PATH}/old"
    for f in "\${FILES_TO_UPDATE[@]}"; do
        mv "\${f}-\${BACKUP_SUFFIX}" "\${ZONE_PATH}/old/"
    done
EOF
# --- FIN DEL BLOQUE SSH ---
