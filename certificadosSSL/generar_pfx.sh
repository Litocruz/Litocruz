...
YEAR=$(date +%Y)
# --- NUEVA VARIABLE PARA EL NOMBRE DE ARCHIVO CON AÑO ---
...
# PASS_FILE está en el DOMAIN_DIR según tu árbol.
...
for file in "$CERT_DIR"/*; do
    case "$file" in
        ...
        *"binary.cer")
            ...
            ;;
        *"Issuer.cer")
            # --- Se mantiene la duplicación de Issuer.cer ---
...
openssl pkcs12 -export -out "${CERT_DIR}/${RENAMED_FILE_NAME}.pfx" -inkey "$KEY_FILE" -in "${CERT_DIR}/${RENAMED_FILE_NAME}.pem" -passout file:"$PASS_FILE"
...
# Eliminar archivos *.Identifier dentro del directorio CERT_DIR
echo "Eliminando archivos temporales Identifier..."
rm -f "$CERT_DIR"/*.Identifier

# Comprimir todo el contenido del dominio en un solo zip 
echo "Comprimiendo todos los archivos generados..."
cd "$BASE_DIR" || exit 1
zip -r "${FILE_NAME}_${YEAR}.zip" "$FILE_NAME"

if [ $? -eq 0 ]; then
    echo "¡Archivos comprimidos con éxito!"
    echo "El archivo zip se ha guardado en: $DOMAIN_DIR/${FILE_NAME}_${YEAR}.zip"
else
